...
- **ניהול עובדים**: הוספה, עריכה ומחיקה של עובדים עם הקצאת תפקידים, הגדרות שעות עבודה מותאמות אישית ושעות חסומות
...
- **ייצוא ל-Google Sheets**: חיבור לחשבון Google וייצוא ישיר של לוחות זמנים ל-Google Sheets
...
נתוני הדוגמה כוללים:
- 5 תפקידי ברירת מחדל (ריפוי בעיסוק, קלינאות תקשורת, פיזיותרפיה, עבודה סוציאלית, טיפול בהבעה ויצירה)
...
## ⚖️ מערכת אימות טיפולים מבוססת שברים (Fractional Validation)

המערכת כוללת מנגנון אימות מתקדם המאפשר תזמון חכם של טיפולים "דו-שבועיים" (כל שבועיים).

### עקרון הפעולה

**ספירה שבריית:**
- טיפול רגיל (שבועי): נספר כ-1.0
- טיפול דו-שבועי: נספר כ-0.5
- קיבולת מקסימלית לחדר/עובד: 1.0

### דוגמאות לתזמון

**✅ מקרים מורשים:**
```
חדר/עובד בזמן 10:00-11:00:
├── טיפול דו-שבועי A (0.5) + טיפול דו-שבועי B (0.5) = 1.0 ✓
├── טיפול רגיל יחיד (1.0) = 1.0 ✓
└── אין טיפולים (0.0) = 0.0 ✓
```
...
```
חדר/עובד בזמן 10:00-11:00:
├── טיפול רגיל (1.0) + טיפול דו-שבועי (0.5) = 1.5 ❌
├── שני טיפולים רגילים (1.0 + 1.0) = 2.0 ❌
└── שלושה טיפולים דו-שבועיים (0.5 × 3) = 1.5 ❌
```

### טיפול בחפיפות מורכבות

כאשר טיפול חדש חופף עם מספר טיפולים קיימים, המערכת בודקת כל חפיפה בנפרד:

**דוגמה:** טיפול חדש 10:00-11:00 עם טיפולים קיימים:
- טיפול A: 09:45-10:30 (דו-שבועי) → חפיפה: 0.5 + 0.5 = 1.0 ✓
- טיפול B: 10:45-11:30 (דו-שבועי) → חפיפה: 0.5 + 0.5 = 1.0 ✓
- טיפול C: 12:00-13:00 (רגיל) → אין חפיפה

**התוצאה:** הטיפול החדש יאושר כי כל חפיפה בנפרד עומדת בגבולות.

### יתרונות המערכת

1. **גמישות מרבית**: מאפשרת ניצול מיטבי של זמני עובדים וחדרים
2. **אימות מדויק**: מונעת התנגשויות תוך מתן אפשרויות תזמון מתקדמות
3. **פשטות בשימוש**: המערכת מטפלת בחישובים באופן שקוף למשתמש
4. **שגיאות ברורות**: הודעות שגיאה מפורטות עם ציון הסכום החורג

### יישום טכני

המערכת כוללת פונקציות אימות טהורות שאינן מבצעות קריאות API:

- `validateRoomFractionalAvailability()` - אימות זמינות חדר
- `validateEmployeeFractionalAvailability()` - אימות זמינות עובד  
- `validatePatientFractionalAvailability()` - אימות זמינות מטופל
- `validateScheduleConstraintsFractional()` - אימות כולל של אילוצי תזמון
- `validatePatientTimeConflictFractional()` - אימות התנגשויות זמן עבור מטופלים

### אימות מטופלים

**אותו עקרון חל גם על מטופלים:**
- מטופל יכול להיות משויך לשני טיפולים דו-שבועיים החופפים
- מטופל לא יכול להיות משויך לטיפול רגיל וטיפול דו-שבועי החופפים  
- מטופל לא יכול להיות משויך לשני טיפולים רגילים החופפים

**דוגמה למטופל:**
```
מטופל בזמן 10:00-11:00:
├── טיפול דו-שבועי A (0.5) + טיפול דו-שבועי B (0.5) = 1.0 ✓
├── טיפול רגיל (1.0) + טיפול דו-שבועי (0.5) = 1.5 ❌
└── שני טיפולים רגילים (1.0 + 1.0) = 2.0 ❌
```
...
המערכת כוללת ניהול מתקדם של עובדים כולל שעות חסומות:

#### שעות חסומות (Reserved Hours)
- **הגדרה גמישה**: כל עובד יכול להגדיר שעות חסומות לכל יום בשבוע
- **פרטי שעות**: הגדרת יום, שעת התחלה, שעת סיום והערות לכל שעה חסומה
- **הגדרות אישיות**: לכל עובד יכול להיות מספר לא מוגבל של שעות חסומות
- **תיקוף אוטומטי**: המערכת מונעת חפיפה בין שעות חסומות של אותו עובד

#### תצוגה בלוח הזמנים
- **הדגשה ויזואלית**: שעות חסומות מוצגות באפור כמו שעות שאינן שעות עבודה
- **אינטראקטיביות**: שעות חסומות עדיין ניתנות ללחיצה ליצירת טיפולים
- **אזהרות**: המערכת מציגה אזהרה בעת יצירת טיפול על שעה חסומה
- **מידע נוסף**: הערות השעה החסומה מוצגות באזהרה

#### ניהול במסך העובדים
- **עמודה ייעודית**: עמודת "שעות חסומות" בטבלת העובדים
- **תצוגה מסודרת**: תגיות צבעוניות המציגות יום ושעות עם הערות בהובר
- **עריכה פשוטה**: הוספה, עריכה והסרה של שעות חסומות בדיאלוג העובד

#### דוגמאות שעות חסומות
נתוני הדוגמה כוללים שעות חסומות לדוגמה:
- ארוחת צהריים (יום ב' 12:00-13:00)
- מפגש צוות (יום ד' 11:00-12:30) 
- פגישה אישית (יום ג' 10:00-11:00)
