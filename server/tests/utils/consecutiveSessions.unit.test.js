...
const mapAPIWeekDayToPrisma = jest.fn().mockImplementation((day) => day.toUpperCase());
// Core validation logic extracted for testing
async function validatePatientConsecutiveSessionsCore(patientId, sessionId, day, startTime, endTime, existingSessions) {
    ...
    const allSessions = [...existingSessions, newSession].sort((a, b) => timeStringToMinutes(a.startTime) - timeStringToMinutes(b.startTime));
...
describe('Consecutive Sessions Validation Logic', () => {
    ...
    it('should pass validation when patient has no existing sessions', async () => {
        const result = await validatePatientConsecutiveSessionsCore('patient-1', 'session-1', 'sunday', '09:00', '09:45', [] // No existing sessions
        );
    ...
    it('should pass validation when patient has two consecutive sessions', async () => {
        ...
        const result = await validatePatientConsecutiveSessionsCore('patient-1', 'session-1', 'sunday', '09:00', '09:45', existingSessions);
        expect(result.valid).toBe(true);
    ...
    it('should throw error when required parameters are missing', async () => {
        await expect(validatePatientConsecutiveSessionsCore('', // Missing patientId
        'session-1', 'sunday', '09:00', '09:45', [])).rejects.toThrow('Missing required parameters');
    });
...
//# sourceMappingURL=consecutiveSessions.unit.test.js.map